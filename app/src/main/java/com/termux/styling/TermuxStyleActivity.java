...
public class TermuxStyleActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        colorSpinner.setOnClickListener(v -> {
            final AlertDialog dialog = new AlertDialog.Builder(TermuxStyleActivity.this).setAdapter(colorAdapter, (dialog1, which) -> copyFile(colorAdapter.getItem(which), true)).create();
        ...
        fontSpinner.setOnClickListener(v -> {
            final AlertDialog dialog = new AlertDialog.Builder(TermuxStyleActivity.this).setAdapter(fontAdapter, (dialog12, which) -> copyFile(fontAdapter.getItem(which), false)).create();
            dialog.setOnShowListener(dialogInterface -> {
                ListView lv = dialog.getListView();
                lv.setOnItemLongClickListener((adapterView, view, position, id) -> {
